    ...
    with conn() as db:
        ...
        """,
            (
                ...
                member.get("nick"),
                bool(user.get("bot")),
                bool(user.get("system")),
                user.get("created_at"),
                member.get("joined_at"),
                user.get("avatar_url"),
                user.get("banner_url"),
                user.get("accent_color"),
                member.get("premium_since"),
                top_role.get("id"),
                top_role.get("name"),
                member.get("status"),
                user.get("public_flags"),
                member.get("communication_disabled_until"),
                derived.get("veteran_rank"),
                derived.get("portrait_asset"),
                ctx.get("invite_code"),
                ctx.get("inviter_id"),
                ctx.get("invite_channel_id"),
                int(derived.get("risk_score", 0)),
                json.dumps(derived.get("risk_reasons", [])),
                json.dumps(snapshot),
            ),
        )
        ...
        for r in roles:
            db.execute(
                "INSERT OR IGNORE INTO player_roles (discord_id,role_id,role_name) VALUES (?,?,?)",
