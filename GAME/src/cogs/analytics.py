...
class Analytics(commands.Cog):
    ...
    @app_commands.command(
        name="topchannels", description="Show most active channels by message count"
    )
    @app_commands.describe(
        days="Window in days (7, 30, or 0 for all)", limit="How many rows to show"
    )
    async def topchannels(self, interaction: discord.Interaction, days: int = 7, limit: int = 10):
        ...
        items = sorted(counts.items(), key=lambda kv: kv[1][0], reverse=True)[
            : max(1, min(25, limit))
        ]
        if not items:
