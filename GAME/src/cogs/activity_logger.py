    ...
    try:
        with sqlite3.connect(DB_PATH) as con:
            con.execute(
                ...
                (_utcnow(), int(user_id), int(guild_id), kind, json.dumps(payload, separators=(',',':')))
            )
    except Exception:
        pass
...
def _attachment_type(att: discord.Attachment) -> str:
    ...
    if ct: return ct.split(";")[0]
    name = (att.filename or "").lower()
    for ext, tag in ((".png","image"),(".jpg","image"),(".jpeg","image"),(".gif","image"),
                     (".mp4","video"),(".mov","video"),(".webm","video"),
                     (".mp3","audio"),(".wav","audio"),(".flac","audio")):
        if name.endswith(ext): return tag
    return "file"
...
class ActivityLogger(commands.Cog):
    ...
    async def on_raw_message_edit(self, payload: discord.RawMessageUpdateEvent):
        ...
        _insert("message_edit", int(data.get("author", {}).get("id", 0)) or 0, guild_id, {
    ...
    async def on_raw_message_delete(self, payload: discord.RawMessageDeleteEvent):
        ...
        _insert("message_delete", int((cached or {}).get("author_id") or 0), payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
            "message_id": str(payload.message_id),
    ...
    async def on_raw_bulk_message_delete(self, payload: discord.RawBulkMessageDeleteEvent):
        ...
        _insert("message_bulk_delete", 0, payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        _insert("reaction_add", payload.user_id, payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
            "message_id": str(payload.message_id),
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        _insert("reaction_remove", payload.user_id, payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
            "message_id": str(payload.message_id),
            "emoji": {"name": payload.emoji.name, "id": str(payload.emoji.id) if payload.emoji.id else None, "animated": payload.emoji.animated},
        })
    ...
    async def on_raw_reaction_clear(self, payload: discord.RawReactionClearEvent):
        _insert("reaction_clear", 0, payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
            "message_id": str(payload.message_id),
        })
    ...
    async def on_raw_reaction_clear_emoji(self, payload: discord.RawReactionClearEmojiEvent):
        _insert("reaction_clear_emoji", 0, payload.guild_id or 0, {
            "channel_id": str(payload.channel_id),
            "message_id": str(payload.message_id),
            "emoji": {"name": payload.emoji.name, "id": str(payload.emoji.id) if payload.emoji.id else None, "animated": payload.emoji.animated},
        })
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        if str(before.status) != str(after.status):
            _insert("presence", after.id, after.guild.id, {"before": str(before.status), "after": str(after.status)})
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if before.channel != after.channel:
            _insert("voice_channel", member.id, g, {
    ...
    # -------- pins --------
    @commands.Cog.listener()
    async def on_guild_channel_pins_update(self, channel: discord.abc.GuildChannel, last_pin: datetime | None):
        gid = getattr(getattr(channel, "guild", None), "id", 0)
        if gid:
            _insert("pins_update", 0, gid, {"channel_id": str(channel.id), "last_pin_utc": last_pin.isoformat().replace("+00:00","Z") if last_pin else None})

    # -------- guild scheduled events --------
    @commands.Cog.listener()
    async def on_scheduled_event_create(self, event: discord.ScheduledEvent):
        if event.guild:
            _insert("scheduled_event_create", 0, event.guild.id, {"id": str(event.id), "name": event.name})

    @commands.Cog.listener()
    async def on_scheduled_event_delete(self, event: discord.ScheduledEvent):
        if event.guild:
            _insert("scheduled_event_delete", 0, event.guild.id, {"id": str(event.id), "name": event.name})
    ...
    async def on_guild_emojis_update(self, guild: discord.Guild, before, after):
        ...
        _insert("emoji_update", 0, guild.id, {
    ...
    async def on_guild_stickers_update(self, guild: discord.Guild, before, after):
        ...
        _insert("sticker_update", 0, guild.id, {
            "added": [str(i) for i in (after_ids - before_ids)],
            "removed": [str(i) for i in (before_ids - after_ids)]
        })
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        diffs = {}
        for attr in ("name", "icon", "banner", "verification_level", "afk_timeout", "vanity_url_code"):
            b = getattr(before, attr, None); a = getattr(after, attr, None)
            if b != a:
                diffs[attr] = {"before": str(b), "after": str(a)}
        if diffs:
    ...
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
        ...
        for attr in ("name", "topic", "nsfw"):
            b = getattr(before, attr, None); a = getattr(after, attr, None)
            if b != a: diffs[attr] = {"before": b, "after": a}
        try:
        ...
        try:
            ...
            if entry.changes:
                for c in entry.changes:
                    changes.append({
                        "attr": c.attribute, "before": getattr(c, "before", None), "after": getattr(c, "after", None)
                    })
            payload = {
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if added or removed:
            _insert("roles", after.id, g, {"added": [str(i) for i in added], "removed": [str(i) for i in removed]})
        ...
        if gained or lost:
            ...
            if self.alert_channel_id and gained:
                ch = after.guild.get_channel(self.alert_channel_id) or self.bot.get_channel(self.alert_channel_id)
                if isinstance(ch, discord.abc.Messageable):
                    ...
                    if gained: e.add_field(name="Gained", value=", ".join(gained), inline=False)
                    if lost:   e.add_field(name="Lost",   value=", ".join(lost),   inline=False)
        ...
        bto = getattr(before, "communication_disabled_until", None) or getattr(before, "timed_out_until", None)
        ato = getattr(after, "communication_disabled_until", None) or getattr(after, "timed_out_until", None)
        if bto != ato:
            _insert("timeout", after.id, g, {
                "before": bto.isoformat().replace("+00:00","Z") if bto else None,
                "after":  ato.isoformat().replace("+00:00","Z") if ato else None,
            })
        if before.premium_since != after.premium_since:
            _insert("boost", after.id, g, {
                "before": before.premium_since.isoformat().replace("+00:00","Z") if before.premium_since else None,
                "after":  after.premium_since.isoformat().replace("+00:00","Z") if after.premium_since else None,
            })
    ...
    async def on_invite_create(self, invite: discord.Invite):
        if invite.guild:
            _insert("invite_create", invite.inviter.id if invite.inviter else 0, invite.guild.id, {
                "code": invite.code, "channel_id": str(getattr(invite.channel, "id", "")), "uses": invite.uses or 0
            })
    ...
    async def on_thread_create(self, thread: discord.Thread):
        if thread.guild:
            _insert("thread_create", thread.owner_id or 0, thread.guild.id, {"thread_id": str(thread.id), "parent_id": str(thread.parent_id)})

    @commands.Cog.listener()
    async def on_thread_delete(self, thread: discord.Thread):
        if thread.guild:
            _insert("thread_delete", thread.owner_id or 0, thread.guild.id, {"thread_id": str(thread.id)})

    @commands.Cog.listener()
    async def on_thread_member_join(self, member: discord.ThreadMember):
        if member.thread and member.thread.guild:
            _insert("thread_member_join", member.id, member.thread.guild.id, {"thread_id": str(member.thread.id)})

    @commands.Cog.listener()
    async def on_thread_member_remove(self, member: discord.ThreadMember):
        if member.thread and member.thread.guild:
            _insert("thread_member_remove", member.id, member.thread.guild.id, {"thread_id": str(member.thread.id)})

    # -------- channel/role/webhook/meta --------
    @commands.Cog.listener()
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        if channel.guild:
            _insert("channel_create", 0, channel.guild.id, {"channel_id": str(channel.id), "type": channel.__class__.__name__})

    @commands.Cog.listener()
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        if channel.guild:
            _insert("channel_delete", 0, channel.guild.id, {"channel_id": str(channel.id), "type": channel.__class__.__name__})
