...
class AuditCmds(commands.Cog):
    ...
    async def audit_recent(self, interaction: discord.Interaction, limit: int = 20):
        ...
        rows = con.execute(
            """SELECT id,type,subject,created_at FROM events ORDER BY id DESC LIMIT ?""",
            (min(limit, 200),),
        ).fetchall()
        lines = [
            f"`{r['id']:>6}`  **{r['type']}**  ⟶  {r['subject'] or '-'}  ·  {r['created_at']}"
            for r in rows
        ]
    ...
    async def audit_user(
        self, interaction: discord.Interaction, user: discord.Member, limit: int = 20
    ):
        await interaction.response.defer(ephemeral=True)
        con = sqlite3.connect(SETTINGS.db_path)
        con.row_factory = sqlite3.Row
        rows = con.execute(
            """SELECT id,type,subject,created_at FROM events
                              WHERE actor_discord_id=? ORDER BY id DESC LIMIT ?""",
            (str(user.id), min(limit, 200)),
        ).fetchall()
        lines = [
            f"`{r['id']:>6}`  **{r['type']}**  ⟶  {r['subject'] or '-'}  ·  {r['created_at']}"
            for r in rows
