...
def allowed_grapple_moves(state: GrappleState) -> set[str]:
    if state == GrappleState.NONE: return {"choke"}
    if state == GrappleState.CHOKING: return {"gouge"}
    if state == GrappleState.GOUGE_WINDOW: return {"push"}
    if state == GrappleState.PUSHED: return set()
    return set()
